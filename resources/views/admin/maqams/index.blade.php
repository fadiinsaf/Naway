    ...
    <div x-data="{
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-primary/5 text-xs uppercase tracking-wider text-primary border-b border-primary/20">
                        <th class="p-4">Maqam</th>
                        <th class="p-4">Description</th>
                        <th class="p-4">Audio / Scale Preview</th>
                ...
                <tbody class="divide-y divide-primary/10 text-sm">
                    ...
                    <tr class="hover:bg-primary/5 transition group">
                        <td class="p-4 font-bold text-accent dark:text-soft text-base">{{ $maqam->name }}</td>
                        <td class="p-4">
                            @if($maqam->description)
                                <p class="opacity-80 text-xs max-w-[200px] truncate mb-1">{{ Str::limit($maqam->description, 50) }}</p>
                                <button @click="currentItem = {{ json_encode($maqam) }}; descModalOpen = true" class="text-xs text-primary hover:underline font-medium">Read more</button>
                            @else
                        ...
                        <td class="p-4">
                            <div class="flex items-center gap-3">
                                <button
                                    @if($maqam->audio_url)
                                        @click="if(playingUrl === '{{ $maqam->audio_url }}') { playingUrl = null; } else { playingUrl = '{{ $maqam->audio_url }}'; }"
                                    @endif
                                    class="w-8 h-8 rounded-full flex items-center justify-center transition {{ $maqam->audio_url ? 'bg-primary/10 text-primary hover:bg-primary hover:text-soft cursor-pointer' : 'bg-gray-200 text-gray-400 dark:bg-gray-800 cursor-not-allowed opacity-50' }}">
                                    <i class="fa-solid text-xs" :class="playingUrl === '{{ $maqam->audio_url }}' ? 'fa-pause' : 'fa-play ml-0.5'"></i>
                                </button>
                                <span class="opacity-80 text-xs font-medium">{{ $maqam->audio_url ? 'Audio Uploaded' : 'No audio' }}</span>
                                <span class="mx-2 opacity-30">|</span>
                                <span class="opacity-80 text-xs font-medium">{{ $maqam->score_url ? 'Scale Uploaded' : 'No Scale' }} {!! $maqam->score_url ? '<i class="fa-solid fa-check text-green-500 ml-1"></i>' : '' !!}</span>
                            </div>
                        </td>
                        <td class="p-4 text-right">
                            <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button @click="currentItem = {{ json_encode($maqam) }}; editModalOpen = true" class="px-3 py-1.5 bg-primary/10 text-primary hover:bg-primary hover:text-soft rounded-lg transition text-xs font-bold">Edit</button>
                                <button @click="currentItem = {{ json_encode($maqam) }}; deleteUrl = '{{ route('admin.maqams.destroy', $maqam) }}'; deleteModalOpen = true" class="px-3 py-1.5 border border-red-500/50 text-red-500 hover:bg-red-500 hover:text-soft rounded-lg transition text-xs font-bold">Delete</button>
                            </div>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="3" class="p-4 text-center opacity-70">No maqams found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div x-show="createModalOpen" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm" x-transition.opacity>
            <div @click.away="createModalOpen = false" class="bg-soft dark:bg-darkbg rounded-2xl border border-primary/30 p-6 max-w-lg w-full mx-4 shadow-2xl relative" x-transition.scale.90>
                <h3 class="text-xl font-bold text-accent dark:text-soft mb-6">Add Maqam</h3>
                <form action="{{ route('admin.maqams.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium mb-1">Name <span class="text-red-500">*</span></label>
                            <input type="text" name="name" placeholder="Maqam Name" required class="w-full bg-white dark:bg-black/20 border border-primary/30 rounded-xl p-2.5 text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary text-accent dark:text-soft">
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Description <span class="text-red-500">*</span></label>
                            <textarea required name="description" rows="3" placeholder="Maqam Description" class="w-full bg-white dark:bg-black/20 border border-primary/30 rounded-xl p-2.5 text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary text-accent dark:text-soft resize-none"></textarea>
                        </div>
                        <div>
                        ...
                        <div>
                            <label class="block text-sm font-medium mb-1">Scale Image <span class="text-red-500">*</span></label>
                    ...
                    <div class="mt-8 flex justify-end gap-3">
                        ...
                        <button type="submit" class="px-5 py-2 rounded-lg text-sm font-medium bg-primary text-soft hover:bg-accent transition shadow-sm">Create Maqam</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div x-show="editModalOpen" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm" x-transition.opacity>
            <div @click.away="editModalOpen = false" class="bg-soft dark:bg-darkbg rounded-2xl border border-primary/30 p-6 max-w-lg w-full mx-4 shadow-2xl relative" x-transition.scale.90>
                <h3 class="text-xl font-bold text-accent dark:text-soft mb-6">Edit Maqam</h3>
                <form :action="currentItem ? '/admin/maqams/' + currentItem.id : '#'" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="space-y-4">
                        <div>
                        ...
                        <div>
                            <label class="block text-sm font-medium mb-1">Scale Image (Leave empty to keep current)</label>
        ...
        <div x-show="deleteModalOpen" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm" x-transition.opacity>
            <div @click.away="deleteModalOpen = false" class="bg-soft dark:bg-darkbg rounded-2xl border border-red-500/30 p-6 max-w-md w-full mx-4 shadow-2xl relative" x-transition.scale.90>
                <h3 class="text-xl font-bold text-red-600 dark:text-red-400 mb-2">Delete Maqam</h3>
