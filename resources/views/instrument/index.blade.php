...
@section('content')
    ...
    @if($instruments->isEmpty())
        <div class="flex flex-col items-center justify-center h-64 text-accent/50 dark:text-soft/50">
            <i class="fa-solid fa-guitar text-4xl mb-4"></i>
            <p>No instruments for a moment we will add instruments soon</p>
        </div>
    @else
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($instruments as $instrument)
                <div x-data="{ liked: {{ auth()->check() && $instrument->likes()->where('user_id', auth()->id())->exists() ? 'true' : 'false' }} }"
                    class="relative bg-soft dark:bg-darkbg rounded-xl shadow border border-primary/30 overflow-hidden flex flex-col">

                    @if($instrument->image)
                        <img src="{{ asset($instrument->image) }}" class="w-full h-48 object-cover">
                    @else
                        <div class="w-full h-48 bg-primary/20 flex items-center justify-center">
                            <i class="fa-solid fa-guitar text-4xl text-primary/50"></i>
                        </div>
                    @endif
                    ...
                    <div class="p-4 flex flex-col flex-grow">
                        <div class="flex-grow flex flex-col">
                            <h3 class="font-bold text-xl text-accent dark:text-soft mb-1">{{ $instrument->name }}</h3>

                            @if($instrument->type || $instrument->origin)
                                <div class="flex flex-wrap gap-2 mb-3">
                                    @if($instrument->type)
                                        <span class="px-2 py-1 text-xs font-semibold rounded bg-primary/20 text-primary dark:text-soft">{{ $instrument->type }}</span>
                                    @endif
                                    @if($instrument->origin)
                                        <span class="px-2 py-1 text-xs font-semibold rounded bg-accent/10 text-accent/80 dark:text-soft/80"><i class="fa-solid fa-globe mr-1"></i>{{ $instrument->origin }}</span>
                                    @endif
                                </div>
                            @endif

                            <p class="text-sm text-accent/70 dark:text-soft/70 mb-4 line-clamp-3">
                                {{ $instrument->historical_description ?? 'No description available.' }}
                            </p>
                        </div>

                        <div class="mt-auto pt-4 border-t border-primary/10">
                            <a href="{{ route('instruments.show', $instrument->id) }}" class="block w-full text-center bg-primary text-soft py-2 rounded hover:bg-accent transition font-semibold">
                                View Details
                            </a>
                        </div>
                    </div>
